...
# --- App Configuration ---
st.set_page_config(page_title="Agro Guard: Smart plant Care System", layout="wide")
...
def load_model_pipeline():
    try:
        ...
        # Creating feature extractor using Model API
...
def predict_image(image):
    ...
    try:
        ...
        if exp_scores.sum() == 0:
             probs = exp_scores # handle ZeroDivision if ever needed, though exp avoids it
        else:
...
# ============================
# 💧 SMART WATERING MERGED CODE
# ============================

if st.session_state.active_tab == "💧 Smart Watering":

    st.header("Smart Watering Dashboard")
    st.write("Monitor your plant environment and control irrigation. Automatically switches between Firebase Live Mode & Simulation Mode.")
    ...
    # -------------------------------------------------------
    # 2️⃣ KPI METRICS (Firebase → Real | Else → Simulated)
    # -------------------------------------------------------
    if live_mode:
        st.success("🟢 LIVE MODE: Firebase IoT Connected")
        ...
        st.warning("🟡 SIMULATION MODE: Firebase Not Connected")
    ...
    # -------------------------------------------------------
    # 3️⃣ PUMP CONTROL (Firebase → Real | Else → Simulated)
    # -------------------------------------------------------
    st.subheader("System Control")

    if live_mode:
        if st.button("▶️ Activate Pump (Firebase)"):
            try:
        ...
        # Simulation pump
        if st.button("▶️ Activate Pump (Simulated)", disabled=st.session_state["pump_running"]):
    ...
    # -------------------------------------------------------
    # 4️⃣ SENSOR HISTORY (Firebase → Simple Chart | Else → Simulation History)
    # -------------------------------------------------------
    st.subheader("Sensor Data (Live)")
    ...
    if live_mode:
        ...
        chart_data = {
            "Moisture (%)": [p["moisture"] for p in hist],
            "Temperature (°C)": [p["temp"] for p in hist],
            "Humidity (%)": [p["humidity"] for p in hist],
        }
    ...
    # -------------------------------------------------------
    # 5️⃣ ACTIVITY LOG (Firebase → IoT log | Else → Local log)
    # -------------------------------------------------------
