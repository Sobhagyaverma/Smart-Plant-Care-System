...
MODEL_DIR = 'plant_disease_model_savedmodel'
MODEL_ZIP_PATH = 'plant_disease_model.zip'
# IMPORTANT: Replace this with the direct download link for your .zip file!
MODEL_URL = "YOUR_DIRECT_DOWNLOAD_LINK_FOR_ZIP_FILE_HERE" 
...
def download_and_unzip(url, save_path, extract_dir):
    if not os.path.exists(extract_dir):
        with st.spinner('Downloading and unzipping model... This may take a minute.'):
            try:
                ...
                st.error(f"Error downloading or unzipping model: {e}")
                st.stop()

# --- Load the Model ---
@st.cache_resource
def load_model():
    download_and_unzip(MODEL_URL, MODEL_ZIP_PATH, MODEL_DIR)
